...
Summary: $\hat{y}$ is a random variable with mean $\mu$ and variance $V(\bar{y})$, which depends on $\sigma^2$, so we will use the specically constructed $\hat{V}({\bar{y})}$ to estimate it.

In other words $\hat{V}({\bar{y})}$ is the *standard error* of $\bar{y}$. 
...
There are lots of "Central Limit Theorems" for various situations, include for sampling from finite populations.
...
$$\bar{y} \pm 2 \hat{V}(\bar{y})$$

> * *The Book* calls $2 \hat{V}(\bar{y})$ *the bound on the error of estimation*.
...
The estimated mean is `r mean(cavities)` and $s^2$ is `r round(var(cavities), 3)`, so the standard error is $\sqrt{\frac{`r round(var(cavities), 3)`}{`r n`}\frac{`r N-n`}{`r N`}}$ = `r round(sqrt(var(cavities)*(N-n)/N), 3)`.
